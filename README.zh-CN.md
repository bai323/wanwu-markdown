...
把网页、AI 对话和浏览器插件里的内容整理成 Markdown、美观报告和结构化资产。

不止是 Markdown，也是一份清晰可看的报告和可继续处理的数据资产。
...
万物 Markdown 是一个本地优先的工作台。它不试图替代你的知识库，也不替你决定内容该怎么使用。它只做一件事：把你有权保存的网页、对话和资料，整理成可阅读的 Markdown、可复盘的视觉报告，以及可继续处理的数据文件。

## 为什么做

AI 工作正在变得越来越分叉、越来越依赖工具，也越来越难复盘。一个有价值的回答不再只是几段文字，它还包含来源页面、模型回复、替代分支、中间过程、文件、标签，以及回到原始上下文的路径。

万物 Markdown 想把这些东西沉淀成真正属于你的本地资产：人可以看，agent 也可以继续读。
...
全模型分支对齐是核心报告视图：模型图例、分叉点、当前回答和替代模型回复会并排对齐，让长对话可以被比较，而不只是被保存。

![真实全模型分支对齐报告](docs/cases/real-model-branch-alignment.png)
...
- 需要登录、滚动或等待加载的页面，可以用“打开采集窗口”采集。

## 输出什么

- Markdown：适合阅读、笔记和 Obsidian。
- HTML 分支报告：适合视觉复盘。
- 对话资产包：用 `manifest.json` 串起 Markdown、报告、JSON、JSONL 和附件。
...
打开窗口采集：粘贴 URL 后点击“打开采集窗口”，在打开的浏览器里登录、滚动或等待内容加载完成，再回到工作台点击“采集当前窗口”。这是更安全的新手版实时采集，因为保存前由你确认页面上已经出现了目标内容。
